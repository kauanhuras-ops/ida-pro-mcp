...
1. **Load an IDA type library** when the API is covered. Select the file by target bitness
   ...
   `py_eval` is hidden and `MCP_UNSAFE`, so it cannot run through general `batch`:
...
declare_type({"decls":[
  "struct IUnknown;",
  "struct IUnknown_vtbl { HRESULT (__stdcall *QueryInterface)(struct IUnknown *, const IID *, void **); ULONG (__stdcall *AddRef)(struct IUnknown *); ULONG (__stdcall *Release)(struct IUnknown *); };",
  "struct IUnknown { struct IUnknown_vtbl *lpVtbl; };"
]})
```

This is a complete three-slot `IUnknown` example. For a derived COM interface, submit all
inherited and interface-specific slots in exact header order. Never put a placeholder
comment inside a final vtable declaration: IDA would accept a short, wrong table.
...
explicit and read both results back. A function rename and function type are independent
when both use the address, so one general `batch` can carry them:

```text
batch({"calls":[
  {"id":"name","tool":"rename","arguments":{"batch":{"func":[{"addr":"<thunk or function>","name":"DirectDrawCreate"}]}}},
  {"id":"prototype","tool":"set_type","arguments":{"edits":[{"kind":"function","addr":"<thunk or function>","ty":"HRESULT __stdcall DirectDrawCreate(GUID *lpGUID, LPDIRECTDRAW *lplpDD, IUnknown *pUnkOuter)"}]}}
]})
```
...
set_type({"edits":[{"kind":"local","addr":"<owner>","variable":"dd_surface","ty":"struct IDirectDrawSurface *"}]})
...
locator. The local type call above depends on the rename result, so it must stay separate.
Use `force_recompile` only after the needed writes have succeeded.
