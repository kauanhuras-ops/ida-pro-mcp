...
`analyze_function` already returns a decompile capped at 100 lines and a block summary.
Do not call `decompile` again unless that output is truncated or a full render is needed;
do not call `basic_blocks` unless the summary is not enough. `disasm.asm.lines` is one
newline-delimited string. Page it with `offset` and `max_instructions`. Do not read
unrelated functions for context before the current function gives a reason to do so.
...
```text
set_type({"edits":[{"kind":"function","addr":"<function>","ty":"int parse_config(Config *cfg, const char *path)"}]})
force_recompile({"items":[{"addr":"<function>"}]})
```

A checked function rename and function type are independent when both use `addr`, so they
may share one general `batch`. Keep `force_recompile` after that batch. A local rename and
a later type edit that locates the new local name are dependent and must stay separate.
...
approved mutation; it is an unsafe mutating helper and cannot run through general
`batch`.
